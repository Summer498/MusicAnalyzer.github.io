...
import { rootDistance, regionDistance, newGetDistance, } from "./TPS.js";
/*
...
for (let i = 0; i < 12; i++) {
    for (let j = 0; j < 12; j++) {
        if (regionDistance(i, j) > 6) {
            throw new Error("regionDistance must be in range [0, 6]. received is regionDistance(" + i + "," + j + ") = " + regionDistance(i, j));
        }
    }
}
...
for (let i = 0; i < 7; i++) {
    for (let j = 0; j < 7; j++) {
        if (rootDistance(i, j) > 3) {
            throw new Error("rootDistance must be in range [0, 3] received is rootDistance(" + i + "," + j + ") = " + rootDistance(i, j));
        }
    }
}
{
    ...
    console.log(newGetDistance({ key: 'C', chord_object: c_info }, { key: 'C', chord_object: g_info }));
    console.log(newGetDistance({ key: 'C', chord_object: dm_info }, { key: 'C', chord_object: am_info }));
    console.log(newGetDistance({ key: 'C', chord_object: c_info }, { key: 'C', chord_object: e_info }));
    console.log(newGetDistance({ key: 'C', chord_object: dm_info }, { key: 'C', chord_object: fm_info }));
}
