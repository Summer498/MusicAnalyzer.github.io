...
export const Range = (begin, end, step = 1) => [...Array(Math.abs(end - begin))].map((_, i) => i * step + begin);
export const Zeros = (length) => [...Array(length)].map(e => 0); // eslint-disable-line @typescript-eslint/no-unused-vars
...
export const matTrans = (matrix) => Range(0, matrix[0].length).map(i => Range(0, matrix.length).map(j => matrix[j][i]));
...
/** @deprecated */
Number.prototype.mod = function (m) { return mod(Number(this), m); };
export const bool2number = (b) => b ? 1 : 0;
/** @deprecated */
Boolean.prototype.toNumber = function () { return bool2number(Boolean(this)); };
export const onehot = (array, n = 0) => [...Array(Math.max(Math.max(...array) + 1, n))].map((_, i) => array.includes(i).toNumber());
/** @deprecated */
Array.prototype.onehot = function (n = 0) { return onehot(this, n); };
export const onehotInMod = (array, m = 1) => array.v_mod(m).onehot(m);
/** @deprecated */
...
export const v_mult = (vector1, vector2) => vFunc(vector1, vector2, (a, b) => a * b);
/** @deprecated */
...
export const v_mod = (vector1, vector2) => vFunc(vector1, vector2, (a, b) => a.mod(b));
/** @deprecated */
