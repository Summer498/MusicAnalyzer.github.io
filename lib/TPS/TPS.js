...
import { Interval, Scale_default, Pcset_default, Chord_default } from "../adapters/Tonal.js";
...
export const regionDistance = (src, dst) => {
    const src_chroma = chroma(excludeUndefined(excludeNull(src.tonic)));
    const dst_chroma = chroma(excludeUndefined(excludeNull(dst.tonic)));
...
export const rootDistance = (src, dst) => {
    const interval = castToNumber(Interval.distance(excludeNull(src.tonic), excludeNull(dst.tonic)).slice(0, 1));
