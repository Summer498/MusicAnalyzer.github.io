...
export const newGetDistance = (src_chord_string, dst_chord_string) => {
    const src = src_chord_string;
    const dst = dst_chord_string;
    console.log(src, dst);
    const region_dist = regionDistance(Tonal.Note.chroma(src.key), Tonal.Note.chroma(dst.key));
    console.log("region_dist", region_dist);
    const root_dist = rootDistance(Tonal.Interval.distance(src.key, src.chord_object.tonic)[0], Tonal.Interval.distance(dst.key, dst.chord_object.tonic)[0]);
...
export const getKeyIncludesTheChord = (chord_string) => {
