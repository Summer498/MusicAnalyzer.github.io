...
import { rootDistance_in_chroma_number, regionDistance_in_chroma_number, getDistance, } from "./TPS.js";
/*
...
for (let i = 0; i < 12; i++) {
    for (let j = 0; j < 12; j++) {
        if (regionDistance_in_chroma_number(i, j) > 6) {
            throw new Error("regionDistance must be in range [0, 6]. received is regionDistance(" + i + "," + j + ") = " + regionDistance_in_chroma_number(i, j));
        }
    }
}
...
for (let i = 0; i < 7; i++) {
    for (let j = 0; j < 7; j++) {
        if (rootDistance_in_chroma_number(i, j) > 3) {
            throw new Error("rootDistance must be in range [0, 3] received is rootDistance(" + i + "," + j + ") = " + rootDistance_in_chroma_number(i, j));
        }
    }
}
...
console.log(getDistance(new RomanChord(Scale_default.get("C major"), Chord_default.get("C")), new RomanChord(Scale_default.get("C major"), Chord_default.get("E"))));
console.log(getDistance(new RomanChord(Scale_default.get("C major"), Chord_default.get("Dm")), new RomanChord(Scale_default.get("C major"), Chord_default.get("Fm"))));
