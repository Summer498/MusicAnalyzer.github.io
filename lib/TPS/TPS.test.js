...
for (let i = 0; i < 21; i++) {
    for (let j = 0; j < 21; j++) {
        const distance = regionDistance(Scale_default.get(`${allSymbols[i]} major`), Scale_default.get(`${allSymbols[j]} major`));
...
for (let i = 0; i < 21; i++) {
    for (let j = 0; j < 21; j++) {
        const distance = tonicDistance(Chord_default.get(allSymbols[i]), Chord_default.get(allSymbols[j]));
        if (distance < -3 || 3 < distance) {
            throw new Error(`rootDistance must be in range [0, 3] received is rootDistance(${i}, ${j}) = ${distance}`);
        }
        const diff = AtoG.indexOf(allSymbols[i].slice(0, 1))
